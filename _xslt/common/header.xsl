...
<xsl:stylesheet exclude-result-prefixes="tei edate xd" extension-element-prefixes="edate" version="1.0" xmlns:edate="http://exslt.org/dates-and-times"
	xmlns:tei="http://www.tei-c.org/ns/1.0" xmlns:xd="http://www.pnp-software.com/XSLTdoc" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="generateAuthor">
		<xsl:choose>
			<xsl:when test="$useHeaderFrontMatter='false' and ancestor-or-self::tei:TEI/tei:text/tei:front//tei:docAuthor">
				<xsl:apply-templates mode="author" select="ancestor-or-self::tei:TEI/tei:text/tei:front//tei:docAuthor[1]"/>
			</xsl:when>
	...
	<xsl:template name="generateDate">
		<xsl:choose>
			...
			<xsl:when test="ancestor-or-self::tei:TEI/tei:teiHeader/tei:fileDesc/tei:editionStmt/tei:edition">
				<xsl:apply-templates select="ancestor-or-self::tei:TEI/tei:teiHeader/tei:fileDesc/tei:editionStmt/tei:edition"> </xsl:apply-templates>
			</xsl:when>
		</xsl:choose>
	</xsl:template>
	
	<!-- I changed this substantially to try to get the proper title on the bar of the web page.  Manipulate that here. -->

	<xd:doc>
		<xd:short>[common] Generate a title</xd:short>
		<xd:detail> RC: I set the useHeaderFrontMatter as true because we will probably not want to use the full title from a front for this.  To ME,
			useHeaderFrontMatter means "use the teiHeader for all front matter."</xd:detail>
	</xd:doc>
	...
	<xsl:template name="generateSubTitle">
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:for-each select="ancestor-or-self::tei:TEI|ancestor-or-self::tei:teiCorpus">
					<xsl:apply-templates mode="htmlheader" select="tei:teiHeader/tei:fileDesc/tei:titleStmt/tei:title[@type='subordinate']"/>
				</xsl:for-each>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
