...
<xsl:stylesheet exclude-result-prefixes="xd exsl estr edate a fo local rng tei teix"
  ...
  <xd:doc type="stylesheet">
    <xd:short> TEI stylesheet dealing with elements from the drama module, making HTML output. </xd:short>
  ...
  <xsl:template match="tei:sp[@rend='noBreak']">
    <dl>
      <dt class="noBreak">
        <xsl:call-template name="makeAnchor"/>
        <xsl:apply-templates select="tei:speaker"/>
      </dt>
      <dd class="noBreakIndent">
        <xsl:apply-templates select="tei:p | tei:l | tei:lg | tei:seg | tei:ab | tei:stage"/>
      </dd>
    </dl>
  </xsl:template>
