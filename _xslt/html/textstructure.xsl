...
<xsl:stylesheet exclude-result-prefixes="exsl estr edate a fo local dbk xlink rng tei teix xd html"
	extension-element-prefixes="exsl estr edate saxon7 saxon6 redirect" version="1.0" xmlns:xlink="http://www.w3.org/1999/xlink"
	xmlns:dbk="http://docbook.org/ns/docbook" xmlns:rng="http://relaxng.org/ns/structure/1.0" xmlns:tei="http://www.tei-c.org/ns/1.0"
	xmlns:teix="http://www.tei-c.org/ns/Examples" xmlns:a="http://relaxng.org/ns/compatibility/annotations/1.0"
	xmlns:edate="http://exslt.org/dates-and-times" xmlns:estr="http://exslt.org/strings" xmlns:exsl="http://exslt.org/common"
	xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:html="http://www.w3.org/1999/xhtml" xmlns:local="http://www.pantor.com/ns/local"
	xmlns:saxon6="http://icl.com/saxon" xmlns:saxon7="http://saxon.sf.net/" xmlns:xd="http://www.pnp-software.com/XSLTdoc"
	xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:redirect="org.apache.xalan.xslt.extensions.Redirect">
	<xd:doc type="stylesheet">
		<xd:short> TEI stylesheet dealing with elements from the textstructure module, making HTML output. </xd:short>
		<xd:detail> This library is free software; you can redistribute it and/or modify it under the terms of the GNU Lesser General Public License
			...
			MA 02111-1307 USA xs </xd:detail>
	...
	<xsl:template match="/">
		<xsl:if test="contains($processor,'Clark')">
			<xsl:message terminate="yes"> XT is not supported by the TEI stylesheets, as it does not implement the "key" function </xsl:message>
		</xsl:if>


		<!-- RC: multiple file processing. -->

		<xsl:for-each select="Edition/part">
			<xsl:apply-templates select="document(@code)/tei:TEI"/>
		</xsl:for-each>

		<!--	putting image files together -->

		<xsl:for-each select="Edition/image">
			<xsl:apply-templates select="document(@code)/tei:TEI"/>
		</xsl:for-each>
      ...
      -->
			...
			<xsl:when test="not($requestedID='')">
				<xsl:if test="$verbose='true'">
					<xsl:message>case 3: ID <xsl:value-of select="$requestedID"/>, pageLayout <xsl:value-of select="$pageLayout"/></xsl:message>
				</xsl:if>
				<xsl:choose>
	...
	<!-- RC: here is the fundamental text structure -->
	<xd:doc>
	...
	<xsl:template match="tei:TEI">
		...
		<!-- RC: turn this on if processing multiple files, and then process using Xalan. -->
		<xsl:choose>
			<xsl:when test="element-available('redirect:write')">
				<xsl:for-each select=".">
					<redirect:write select="concat($idNumber, '.html')">
						...
						<html xmlns="http://www.w3.org/1999/xhtml">
							...
							<head>
								...
								<!-- <object><xsl:call-template name="COINS"/></object>  -->
							...
							<body>
							  <xsl:choose>
							    <xsl:when test="//tei:pb/@facs">
							        <div id="container" class="wPageImages">
						...
						<!-- RC: turn this on if processing multiple files -->
					</redirect:write>
				</xsl:for-each>
			</xsl:when>
		</xsl:choose>
	...
	<xsl:template match="tei:dateline">
		<p xmlns="http://www.w3.org/1999/xhtml" class="dateline">
			<xsl:apply-templates/>
		</p>
	</xsl:template>
	<xd:doc>
	...
	<xd:doc>
		<xd:short>Process elements tei:docTitle, docAuthor, docImprint and tei:docDate</xd:short>
		<xd:detail>
			<p> Translate these to a corresponding HTML div </p>
		</xd:detail>
	</xd:doc>
	<xsl:template match="tei:docTitle|tei:docAuthor|tei:docImprint|tei:docDate|tei:docEdition|tei:titlePart">
		<div xmlns="http://www.w3.org/1999/xhtml">
			<xsl:choose>
	...
	<xsl:template name="doDivBody">
		...
		<xsl:choose>
			<xsl:when test="parent::tei:div">
				<!-- RCs, here is where there is a difference between the header of the division of the whole essay which contains the section divs and the
				section divs. -->
	...
	<xsl:template name="includeCSS">
		<xsl:choose>
			...
			<xsl:otherwise>
				<link xmlns="http://www.w3.org/1999/xhtml" href="{$cssFile}" rel="stylesheet" type="text/css"/>
			</xsl:otherwise>
		</xsl:choose>
		<xsl:if test="not($cssSecondaryFile='')">
			<link xmlns="http://www.w3.org/1999/xhtml" href="{$cssSecondaryFile}" media="screen" rel="stylesheet" type="text/css" />
		</xsl:if>
		<xsl:if test="not($cssPrintFile='')">
			<link xmlns="http://www.w3.org/1999/xhtml" rel="stylesheet" media="print" type="text/css" href="{$cssPrintFile}"/>
		</xsl:if>
	...
	<xd:doc>
		...
		<xd:detail>Note that it does not have to commented if the output is XHTML</xd:detail>
	</xd:doc>
	...
	<xsl:template name="nextLink">
		...
		<xsl:choose>
			...
			<xsl:when test="$myName='div2' and following-sibling::tei:div2[tei:hea or $autoHead='true']">
		...
		<xsl:choose>
			...
			<xsl:when test="element-available('exsl:document')">
				...
				<xsl:if test="element-available('exsl:document')">
					<exsl:document doctype-public="{$doctypePublic}" doctype-system="{$doctypeSystem}" encoding="{$outputEncoding}" href="{$outName}"
						method="{$outputMethod}">
						<xsl:copy-of select="$content"/>
					</exsl:document>
				</xsl:if>
			...
			<xsl:when test="contains($processor,'SAXON 7')">
				...
				<saxon7:output doctype-public="{$doctypePublic}" doctype-system="{$doctypeSystem}" encoding="{$outputEncoding}" href="{$outName}"
					method="{$outputMethod}">
					<xsl:copy-of select="$content"/>
					<xsl:fallback>
						<xsl:copy-of select="$content"/>
			...
			<xsl:when test="contains($processor,'SAXON 6')">
				...
				<saxon6:output doctype-public="{$doctypePublic}" doctype-system="{$doctypeSystem}" encoding="{$outputEncoding}" href="{$outName}"
					method="{$outputMethod}">
					<xsl:copy-of select="$content"/>
					<xsl:fallback>
						<xsl:copy-of select="$content"/>
			...
			<xsl:when test="contains($processor,'Apache')">
				...
				<xalan:write file="{$outName}" xmlns:xalan="org.apache.xalan.xslt.extensions.Redirect" xsl:extension-element-prefixes="xalan">
					<xsl:copy-of select="$content"/>
					<xsl:fallback>
						<xsl:copy-of select="$content"/>
					</xsl:fallback>
			...
			<xsl:otherwise>
				<xsl:message terminate="yes">Creation of <xsl:value-of select="$outName"/> not possible with <xsl:value-of select="$processor"
					/></xsl:message>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template name="mainPage">
		...
		<div id="accessibility">
			<span class="tocontent"><a href="{$REQUEST}?style=text">Text only</a> | <a class="skiplinks" href="#rh-column"
	...
	<xsl:template name="simpleBody">
		...
		<!-- back matter -->
		<xsl:apply-templates select="tei:text/tei:back"/>
		<!--<hr xmlns="http://www.w3.org/1999/xhtml" size="1" noshade="noshade"/> -->
		<xsl:call-template name="printNotes"/>
	</xsl:template>
	<xd:doc>
		<xd:short>[html] </xd:short>
			...
			<xsl:if test="$linkPanel='true'">
				<div class="footer">
					...
					<xsl:if test="$searchURL"> | <a class="{$style}" href="{$searchURL}">
							...
							<xsl:value-of select="$searchWords"/>
						</a>
					</xsl:if>
					<xsl:if test="$feedbackURL"> | <a class="{$style}" href="{$feedbackURL}">
							...
							<xsl:value-of select="$feedbackWords"/>
						</a>
					</xsl:if>
				</div>
				<hr/>
	...
	<xsl:template name="summaryToc">
		<div class="teidiv">
			<p>Select headings on the left-hand side to see more explanation of the links on the right.</p>
			<table cellspacing="7">
				<thead>
					<tr>
						<th nowrap="nowrap"/>
	...
	<xd:doc>
		...
		<xd:param name="force">whether to force a TOC entry even if other rules would normally prevent it</xd:param>
		<xd:detail> </xd:detail>
	</xd:doc>
						...
						<xsl:choose>
							...
							<xsl:otherwise>
								<a class="{$class}">
									...
									<xsl:attribute name="href"><xsl:value-of select="$whole"/>/<xsl:value-of select="$current"
